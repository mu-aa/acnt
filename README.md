...
## 느낀점
 ...
 - 처음으로 Java에 내장되지 않은  라이브러리(chart.js)를 활용한 프로젝트를 진행했습니다. 값 전달 오류, 라이브러리 함수 검색 등 많은 시행착오를 겪으며 어떤 방식으로 라이브러리를 활용해야 하는지 경험했습니다.
 - 중간에 팀원 두 명이 학원을 그만두어 6인에서 4인으로 인원이 줄어 많은 기능을 혼자 담당해야 했습니다. 하지만 덕분에 촉박한 일정 속에서도 일정을 관리하는 방법을 터득했습니다. 
